...
// Konfigurasi aplikasi kedua untuk manajemen pembuatan akun
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateSemuaDropdown() {
        ...
        const mapelCheckboxes = listMapel.map(m => `<label style="display:block;"><input type="checkbox" class="new-mapel-cb" value="${m}"> ${m}</label>`).join('');
        const kelasCheckboxes = listKelas.map(k => `<label style="display:block;"><input type="checkbox" class="new-kelas-guru-cb" value="${k}"> ${k}</label>`).join('');
        ...
        const emc = document.getElementById('edit-mapel-container'); if (emc) emc.innerHTML = listMapel.map(m => `<label style="display:block;"><input type="checkbox" class="edit-mapel-cb" value="${m}"> ${m}</label>`).join('');
        const ekc = document.getElementById('edit-kelas-guru-container'); if (ekc) ekc.innerHTML = listKelas.map(k => `<label style="display:block;"><input type="checkbox" class="edit-kelas-guru-cb" value="${k}"> ${k}</label>`).join('');
    }
    ...
    // ==========================================
    // 6. MANAJEMEN BANK SOAL 
    // ==========================================
    
    // --- Menampilkan Soal di Tabel ---
    ...
    function renderFormDinamis(tipe) {
        ...
        if (tipe === 'PG' || tipe === 'PGK') {
            ...
            areaOpsi.innerHTML = `<div id="container-jodoh">${[1,2,3].map(num => `<div style="display: flex; gap: 10px; margin-bottom: 8px;"><input type="text" class="jodoh-kiri input-text" placeholder="Pernyataan ${num}"><input type="text" class="jodoh-kanan input-text" placeholder="Jawaban ${num}"></div>`).join('')}</div>`;
        }
    ...
    document.getElementById('btn-dl-excel')?.addEventListener('click', (e) => {
        ...
        const ws = XLSX.utils.aoa_to_sheet(ws_data); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Soal"); XLSX.writeFile(wb, "Template_Soal.xlsx");
    });

    document.getElementById('btn-dl-word')?.addEventListener('click', (e) => {
        e.preventDefault();
        const content = `<html><body><p>NO: 1<br>TIPE: PG<br>SOAL: Contoh Soal?<br>A. Opsi A<br>B. Opsi B<br>KUNCI: A</p></body></html>`;
        const blob = new Blob(['\ufeff', content], { type: 'application/msword' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'Template_Soal.doc'; a.click();
    });
            ...
            reader.onload = async (e) => {
                try {
                    ...
                    for (let i = 0; i < qList.length; i++) {
                        let q = qList[i];
                        btn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> Memproses ${i + 1}/${qList.length}...`;
    ...
    document.getElementById('btn-update-soal').onclick = async () => {
        ...
        const btn = document.getElementById('btn-update-soal'); btn.innerHTML = 'Memperbarui...'; btn.disabled = true;
        try {
    ...
    document.getElementById('btn-preview-full').onclick = () => {
        if(filteredSoalData.length === 0) return customAlert("Tampilkan soal dulu!", "warning");
    ...
    window.openDetailHasil = (mapel) => {
        currentMapelDetail = mapel; window.location.hash = 'section-hasil-detail';
        document.getElementById('label-mapel-detail').innerText = mapel;
    ...
    async function loadActiveTokens() {
        ...
        const tbody = document.querySelector('#table-active-tokens tbody'); if(!tbody) return; tbody.innerHTML = '';
        if(snap.exists()){
            Object.entries(snap.data()).forEach(([k, d]) => {
                let timeLeft = Math.floor((d.expiresAt - Date.now())/60000);
                tbody.innerHTML += `<tr><td>${k.replace('token_','')}</td><td>-</td><td><b>${d.code||d}</b> <span style="font-size:0.7rem; background:${timeLeft>0?'var(--success)':'var(--danger)'}; color:#fff; padding:2px 6px; border-radius:4px;">${timeLeft>0?timeLeft+' mnt':'Habis'}</span></td><td><button onclick="window.hapusTokenUtama('${k}')" style="color:var(--danger); border:none; background:none; cursor:pointer;"><i class="fas fa-trash"></i></button></td></tr>`;
            });
        }
    }
    document.getElementById('btn-refresh-token').onclick = loadActiveTokens;
    document.getElementById('btn-save-token').onclick = async () => {
        const m = document.getElementById('set-token-mapel').value, k = document.getElementById('set-token-kelas').value, t = document.getElementById('input-token-baru').value.toUpperCase();
        if(!m||!k||!t) return customAlert("Isi lengkap!", "warning");
        try { await setDoc(doc(db, "pengaturan", "token_ujian"), { [`token_${m}_${k}`]: { code: t, expiresAt: Date.now() + (15*60000) } }, { merge: true }); customAlert("Token tersimpan (Aktif 15mnt)!", "success"); loadActiveTokens(); } catch(e) {}
    };

    window.hapusTokenUtama = async (k) => { if(await customConfirm("Hapus Token?")) { await updateDoc(doc(db, "pengaturan", "token_ujian"), {[k]: deleteField()}); loadActiveTokens(); } };
