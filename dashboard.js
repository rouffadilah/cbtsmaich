...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadDataPengguna() {
        ...
        try {
            ...
            if (tbodyGuru) tbodyGuru.innerHTML = '<tr><td colspan="5" style="text-align:center; color:var(--danger);">Gagal memuat data</td></tr>';
            if (tbodySiswa) tbodySiswa.innerHTML = '<tr><td colspan="5" style="text-align:center; color:var(--danger);">Gagal memuat data</td></tr>';
        }
    }
    ...
    document.getElementById('btn-save-edit-akun')?.addEventListener('click', async () => {
        ...
        try {
            ...
            // Catatan: Jika password diubah, Anda perlu memanggil fungsi Cloud Function khusus (jika ada) 
            // karena update password di Firebase Auth dari sisi klien hanya bisa dilakukan oleh user yang sedang login.
            if (pass) {
    ...
    async function loadBankSoalSummary() {
        ...
        try {
            ...
            if (statSoalEl) statSoalEl.innerText = snap.size;
    ...
    window.bukaDetailSoal = async (mapel, kelas) => {
        ...
        // Catatan: Anda mungkin memerlukan fungsi loadDataSoal() yang sebenarnya tidak ada pada file yang Anda unggah sebelumnya
        // Di sini saya asumsikan loadBankSoalSummary dipanggil saat "Kembali"
    };
